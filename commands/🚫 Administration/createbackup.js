...
module.exports = {
    ...
    run: async (client, message, args, cmduser, text, prefix) => {
        ...
        if(!message.guild.me.permissions.has(Discord.Permissions.FLAGS.ADMINISTRATOR)){
            return message.reply("<:M_x:933261310588760115> **I am missing the ADMINISTRATOR Permission!**")
        }
        ...
        if(owner.id != cmduser.id) {
            return message.reply(`<:M_x:933261310588760115> **You need to be the Owner of this Server!**`)
        }
        ...
        message.channel.send({
            ...
            collector.on('collect', button => {
                if (button?.user.id === cmduser.id) {
                    collector.stop();
                    button?.reply({content: `<a:Loading:935797567596421120> **Now saving the Backup!**\nThis could take up to 2 Minutes (belongs to your data amount)`}).catch(() => {})
                    // Create the backup
