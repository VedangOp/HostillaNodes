...
module.exports = {
  ...
  run: async (client, message, args, cmduser, text, prefix) => {
    ...
    try {
      ...
      if(kickmember.communicationDisabledUntilTimestamp) return message.reply("<:M_x:933261310588760115> **This User is already timeouted!**");
      let time = 0;
      if(!args[1]) return message.reply(`<:M_x:933261310588760115> **No time added!**\nTry something like this:\n> \`${prefix}timeout ${kickmember.id} 1h+15min Stop spamming!\``)
      let timeargs = [args[1]];
      ...
      if(!time || isNaN(time)) return message.reply(`<:M_x:933261310588760115> **You added a invalid time!**\nTry something like this:\n> \`${prefix}timeout ${kickmember.id} 1h+15min Stop spamming!\``)
      ...
      if (!kickmember.manageable)
        return message.reply({embeds :[new MessageEmbed()
          .setColor(es.wrongcolor)
          .setFooter(client.getFooter(es))
          .setTitle("<:M_x:933261310588760115> **I am not able to manage this User**")
        ]}).catch(()=>{});
      try{
