        ...
        client.on('interactionCreate',  (menu) => {
          if (menu?.message.id === menumsg.id) {
            if (menu?.user.id === cmduser.id) {
              if(used1) return menu?.reply({content: `<:M_x:933261310588760115> You already selected something, this Selection is now disabled!`, ephemeral: true})
              menuselection(menu);
            }
          ...
          .addComponents([button_ae, /*button_nl, button_tr, button_ir*/])
            ...
            collector.on('collect', async b => {
                if(b?.user.id !== message.author.id)
                  return b?.reply(`<:M_x:933261310588760115> **Only the one who typed ${prefix}setup-language is allowed to react!**`, true)
