    ...
    try {
        ...
        let menuoptions = [
          {
            ...
            emoji: `${client.settings.get(message.guild.id, `antidiscord.enabled`) ? "933261310588760115" : "932995837003460649"}`
          },
          {
            value: "Settings",
      ...
      async function handle_the_picks(menuoptionindex, menuoptiondata) {
        switch(menuoptionindex){
          ...
          case 5: {
            let antisettings = client.settings.get(message.guild.id, "antidiscord.whitelistedlinks")
            if(antisettings.length < 1) return message.reply("<:M_x:933261310588760115> There are no links whitelisted...")
            tempmsg = await message.reply({embeds: [new Discord.MessageEmbed()
          ...
          case 6: {
            ...
            .then(collected => {
              ...
              if (message.content) {
                ...
                if(isNaN(number)) return message.reply("<:M_x:933261310588760115> **Not a valid Number**");
                if(Number(number) < 0 || Number(number) > 15) return message.reply("<:M_x:933261310588760115> **The Number must be between `0` and `15`**");
