    ...
    try {
      ...
      async function first_layer() {
        ...
        let menumsg = await message.reply({
          embeds: [MenuEmbed],
          components: [row1, row2, row3, row4, new MessageActionRow().addComponents(new MessageButton().setStyle("LINK").setURL("https://youtu.be/QGESDc31d4U").setLabel("Tutorial Video").setEmoji("932982270481027082"))]
        })
        ...
        collector.on('collect', menu => {
          if (menu?.user.id === cmduser.id) {
            ...
            content: `<:M_x:933261310588760115> You are not allowed to do that! Only: <@${cmduser.id}>`,
            ephemeral: true
          });
        });
        ...
        collector.on('end', collected => {
          menumsg.edit({
            ...
            components: [],
            content: `<a:YES:932995837003460649> **Selected: \`${collected && collected.first() && collected.first().values ? collected.first().values[0] : "Nothing"}\`**`
          }).catch(() => {});
        });
      }
      async function second_layer(SetupNumber) {
        ...
        let menumsg = await message.reply({
          embeds: [MenuEmbed],
          components: [new MessageActionRow().addComponents(Selection), new MessageActionRow().addComponents(new MessageButton().setStyle("LINK").setURL("https://youtu.be/QGESDc31d4U").setLabel("Tutorial Video").setEmoji("932982270481027082"))]
        })
        //Create the collector
        const collector = menumsg.createMessageComponentCollector({
          filter: i => i?.isSelectMenu() && i?.message.author.id == client.user.id && i?.user,
        ...
        switch (optionhandletype) {
            ...
            async function third_layer(SetupNumber) {
              ...
              collector.on('end', collected => {
                menumsg.edit({
                  embeds: [menumsg.embeds[0].setDescription(`~~${menumsg.embeds[0].description}~~`)],
                  components: [],
                  content: `${collected && collected.first() && collected.first().values ? `<a:YES:932995837003460649> **Selected: \`${collected ? collected.first().values[0] : "Nothing"}\`**` : "❌ **NOTHING SELECTED - CANCELLED**" }`
                })
              });
            }
          ...
          case "Send the Config	Message": {
            ...
            if (!data || data.length < 1) {
              return message.reply("<:M_x:933261310588760115> **You need to add at least 1 Open-Ticket-Option**")
            }
            ...
            if (collected && collected.first().content) {
              ...
              if (collected2 && (collected2.first().mentions.channels.size > 0 || message.guild.channels.cache.get(collected2.first().content?.trim()))) {
                ...
                return message.reply("<:M_x:933261310588760115> **You did not ping a valid Channel!**")
              }
            } else {
              return message.reply("<:M_x:933261310588760115> **You did not enter a Valid Message in Time! CANCELLED!**")
            }
          }
          break;
          case "Add Ticket Option": {
            let data = theDB.get(message.guild.id, pre+".data");
            if (data.length >= 25) {
              return message.reply("<:M_x:933261310588760115> **You reached the limit of 25 different Options!** Remove another Option first!")
            }
            ...
            if (collected && collected.first().content) {
              if (!collected.first().content.includes("++")) return message.reply("<:M_x:933261310588760115> **Invalid Usage! Please mind the Usage and check the Example**")
              let value = collected.first().content.split("++")[0].trim().substring(0, 25);
              let index = data.findIndex(v => v.value == value);
              if(index >= 0) {
                  return message.reply("<:M_x:933261310588760115> **Options can't have the SAME VALUE!** There is already an Option with that Value!");
              }
                    ...
                    message.reply({embeds: [rermbed]}).then(async msg => {
                      ...
                      msg.awaitReactions({ filter: (reaction, user) => user.id == cmduser.id, 
                        ...
                        if (collected.first().emoji?.id && collected.first().emoji?.id.length > 2) {
                          ...
                          message.reply("<:M_x:933261310588760115> **No valid emoji added, using default EMOJI**");
                          emoji = null;
                        ...
                        try {
                          ...
                          console.log(e)
                          message.reply("<:M_x:933261310588760115> **Could not use the CUSTOM EMOJI you added, as I can't access it / use it as a reaction/emoji for the menu**\nUsing default emoji!");
                          emoji = null;
                          emojiMsg = NumberEmojis[data.length];
                        }
                        finished();
          ...
          case "Edit Ticket Option": {
            ...
            let data = theDB.get(message.guild.id, pre+".data");
            if (!data || data.length < 1) {
              return message.reply("<:M_x:933261310588760115> **There are no Open-Ticket-Options to remove**")
            }
            let embed = new MessageEmbed()
              .setColor(es.color)
              .setThumbnail(es.thumb ? es.footericon && (es.footericon.includes("http://") || es.footericon.includes("https://")) ? es.footericon : client.user.displayAvatarURL() : null)
                ...
                async function handle_the_picks3(optionhandletype) {
                  ...
                  switch (optionhandletype) {
                    ...
                    case `Change Open Category`:{
                      ...
                      return message.reply("<:M_x:933261310588760115> **Invalid Category-ID added**")
                    }break;
            ...
            collector.on('end', collected => {
              menumsg.edit({
                embeds: [menumsg.embeds[0].setDescription(`~~${menumsg.embeds[0].description}~~`)],
                components: [],
                content: `<a:YES:932995837003460649> **Selected: \`${collected.size > 0 ? collected.first().values[0] : "NOTHING"}\`**`
              })
            });
          }
          break;
          ...
          collector.on('end', collected => {
            menumsg.edit({
              embeds: [menumsg.embeds[0].setDescription(`~~${menumsg.embeds[0].description}~~`)],
              components: [],
              content: `<a:YES:932995837003460649> **Selected: \`${collected.first().values[0]}\`**`
            })
          });
        }
        break;
          ...
          case "Manage General Access": {
            ...
            if (collected && (collected.first().mentions.roles.size > 0 || collected.first().mentions.users.size > 0)) {
              ...
              message.reply("<:M_x:933261310588760115> **You did not ping valid user(s)**")
            }
          }break;
        }
      }
