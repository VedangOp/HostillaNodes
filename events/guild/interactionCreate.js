...
module.exports = async (client, interaction) => {
      ...
      if(!guild) {
        return interaction?.reply({content: "<:M_x:933261310588760115> Interactions only Works inside of GUILDS!", ephemeral: true}).catch(()=>{});
      }
        ...
        if(player && player.queue && player.queue.current && command.parameters.check_dj){
          if(check_if_dj(client, interaction?.member, player.queue.current)) {
            return interaction?.reply({embeds: [new MessageEmbed()
              ...
              .setTitle(` <: no: 933261310588760115 > ** You are not a DJ and not the Song Requester! ** `)
