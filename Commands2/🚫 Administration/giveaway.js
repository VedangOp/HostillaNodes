        ...
        if (args[0].toLowerCase() === "start") {
            try{
                ...
                let options = {
                    ...
                    prize: `<a:prize:934366358437761054> ${giveawayPrize} <a:prize:934366358437761054>`,
                    ...
                    bonusEntries: [],
                    messages: {
                        inviteToParticipate: "***React with <a:cc_giveaway:933256992041091093> to participate!***\n",
                        drawing: "> Ends: {timestamp}\n",
                        hostedBy: "**Hosted by:** {this.hostedBy}",
                        dropMessage: "Be the first to react with <a:cc_giveaway:933256992041091093>",
                        noWinner: "\n**Giveaway cancelled!**\n> No valid participations. :cry:",
                        endedAt: "Ends at", 
                        giveaway: '<a:cc_giveaway:933256992041091093> **GIVEAWAY STARTED** <a:cc_giveaway:933256992041091093>',
                        giveawayEnded: '<a:Tada_Green:943360781754449930> **GIVEAWAY ENDED** <a:Tada_Green:943360781754449930>',
                ...
                if(bonusentriesdata.mentions.roles.size > 0){
                    ...
                    if(bonusentriesdata.mentions.roles.size > 1){
                        if(!args[0]) return message.reply("<:M_x:933261310588760115> Invalid Input of Multiple Bonus Roles, check the EXAMPLE!")
            ...
            client.giveawaysManager.reroll(giveaway.messageId, { winnerCount: !isNaN(args[1]) ? Number(args[1]) : 1})
                ...
                .catch((e) => {
                    if (e.startsWith(`Giveaway with message Id ${giveaway.messageId} is not ended.`)) {
                        ...
                        console.error(e);
                        message.reply({content : '<:M_x:933261310588760115> **An error occured...**```' + String(e.message).substring(0, 1900) + "```"});
                    }
                });
            ...
            args.shift();
            if (!args[0]) {
                return message.reply({content : `<:M_x:933261310588760115> The right usage of this Command is: \`${prefix}giveaway winner <GiveawayId>\` ... note that GiveawayId is the MessageId of the (Embed) Giveaway-Message`});
            }
            let giveaway = client.giveawayDB.find((g) => g.messageId === args[0]);

            if (!giveaway) {
                return message.reply({content : "<:M_x:933261310588760115> Could not find Data of this Giveaway"});
            }
