...
let buttonEvent = async (interaction) => {
  ...
  if (interaction?.message.id === msg.id) {
    if (interaction?.user.id === message.member.id) {
      ...
      await interaction?.reply({content : `<:M_x:933261310588760115> You are not allowed to do that! Only: <@${message.author.id}>`, ephemeral : true}); //ephemeral message
    }
  }
}
...
let embedEditing = async(button) => {
    ...
    if(id.startsWith(`builder`)) {
        ...
        button?.message.edit({content : `<a:Loading:935797567596421120> **Please send me Your Input now!**`,
