...
module.exports = async client => {
    ...
    client.on("interactionCreate", async interaction => {
        ...
        user.send({
            ...
            if(!Platform) {
                return user.send("<:M_x:933261310588760115> Cancelled, due to no reaction in under 2 Minutes!")
            }
            ...
            if(!Username) {
                return user.send("<:M_x:933261310588760115> Cancelled, due to not sending the Username in under 2 Minutes!")
            }
            let others = client.epicgamesDB.find(d => d.guild && d.guild == guildId && d.epic && d.epic == Username);
            if(others && others.length > 0) return user.send(`<:M_x:933261310588760115> **Someone with the User-ID: \`${others.user}\` Linked their Account with this Epic Games Name!**`) 
            ...
            if(!tdata || tdata.code === 404) {
                return user.send("<:M_x:933261310588760115> Could not find your Epic Games Account, please try again and make sure you send the right name!")
            }
