...
module.exports = client => {
  ...
  async function ApplySystem({ guild, channel, user, message, interaction, es, ls, preindex = false }) {
    ...
    if(!index) {
      if(!interaction.replied) return interaction?.reply({ephemeral: true, content: `<:M_x:933261310588760115> Could not find the Database for your Application!`});
      else return interaction?.editReply({ephemeral: true, content: `<:M_x:933261310588760115> Could not find the Database for your Application!`});
    }
