        ...
        try{
            ...
            if(message.channel.id == chatbot){
              ...
              try{
                ...
                message.channel.send({content: "<:M_x:933261310588760115> AI CHAT API IS DOWN"}).catch(() => {})
              }
            }
        ...
        try{
            ...
            for(const user of [...message.mentions.users.values()]){
                if(client.afkDB.has(message.guild.id + user.id)){
                    await message.reply({content: `<a:Attention:933256066920243241> **${user.tag}** went AFK <t:${Math.floor(client.afkDB.get(message.guild.id+user.id, "stamp") / 1000)}:R>!${client.afkDB.get(message.guild.id+user.id, "message") && client.afkDB.get(message.guild.id+user.id, "message").length > 1 ? `\n\n__His Message__\n>>> ${String(client.afkDB.get(message.guild.id+user.id, "message")).substring(0, 1800).split(`@`).join(`\`@\``)}` : "" }`}).then(msg=>{
                        setTimeout(()=>{
                            try{
                                msg.delete().catch(() => {});
                            }catch{  }
                    ...
                    try { 
                        if(!message.deleted) {
                            if(message.channel.permissionsFor(message.channel.guild.me).has(Permissions.FLAGS.MANAGE_MESSAGES)){
                                ...
                                message.reply("<:M_x:933261310588760115> **I am missing the MANAGE_MESSAGES Permission!**").then(m => {
