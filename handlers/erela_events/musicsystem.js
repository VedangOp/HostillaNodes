...
module.exports = client => {
    client.on("interactionCreate", async (interaction) => {
        ...
        if(!member.voice.channel) return interaction?.reply({ephemeral: true, content: "<:M_x:933261310588760115> **Please Connect to a Voice Channel first!**"})
        //now its time to start the music system
        if (!member.voice.channel)
            return interaction?.reply({
                content: `<:M_x:933261310588760115> **Please join a Voice Channel first!**`,
                ephemeral: true
            })      
            
        var player = client.manager.players.get(interaction?.guild.id);
        if (interaction?.customId != "Join" && interaction?.customId != "Leave" && (!player || !player.queue || !player.queue.current))
            return interaction?.reply({content: "<:M_x:933261310588760115> Nothing Playing yet", ephemeral: true})
        ...
        if (player && member.voice.channel.id !== player.voiceChannel)
            return interaction?.reply({
                content: `<:M_x:933261310588760115> **Please join __my__ Voice Channel first! <#${player.voiceChannel}>**`,
                ephemeral: true
            })
        ...
        if(player && interaction?.customId != `Join` && interaction?.customId != `Lyrics` && check_if_dj(client, member, player.queue.current)) {
                return interaction?.reply({embeds: [new MessageEmbed()
                  ...
                  .setTitle(`<:M_x:933261310588760115> **You are not a DJ and not the Song Requester!**`)
        ...
        switch(interaction?.customId){
            ...
            case "Autoplay": {
                ...
                interaction?.reply({
                  ...
                  .setColor(ee.color)
                  .setTimestamp()
                  .setTitle(`${player.get(`autoplay`) ? `<a:YES:932995837003460649> **Enabled Autoplay**`: `<:M_x:933261310588760115> **Disabled Autoplay**`}`)
                  .setFooter(client.getFooter(`💢 Action by: ${member.user.tag}`, member.user.displayAvatarURL({dynamic: true})))]
                })
                //edit the message so that it's right!
                var data = generateQueueEmbed(client, guild.id)
            ...
            case "Song": {
                ...
                interaction?.reply({
                  ...
                  .setColor(ee.color)
                  .setTimestamp()
                  .setTitle(`${player.trackRepeat ? `<a:YES:932995837003460649> **Enabled Song Loop**`: `<:M_x:933261310588760115> **Disabled Song Loop**`}`)
                  .setFooter(client.getFooter(`💢 Action by: ${member.user.tag}`, member.user.displayAvatarURL({dynamic: true})))]
                })
                //edit the message so that it's right!
                var data = generateQueueEmbed(client, guild.id)
            ...
            case "Queue": {
                ...
                interaction?.reply({
                  ...
                  .setColor(ee.color)
                  .setTimestamp()
                  .setTitle(`${player.queueRepeat ? `<a:YES:932995837003460649> **Enabled Queue Loop**`: `<:M_x:933261310588760115> **Disabled Queue Loop**`}`)
                  .setFooter(client.getFooter(`💢 Action by: ${member.user.tag}`, member.user.displayAvatarURL({dynamic: true})))]
                })
                //edit the message so that it's right!
                var data = generateQueueEmbed(client, guild.id)
    ...
    client.on("messageCreate", async message => {
        ...
        if (prefixRegex.test(message.content)) {
            ...
            if (cmd || cmd.length === 0) return// message.reply("<:M_x:933261310588760115> **Please use a Command Somewhere else!**").then(msg=>{setTimeout(()=>{try{msg.delete().catch(() => {});}catch(e){ }}, 3000)})
            ...
            {
                return// message.reply("<:M_x:933261310588760115> **Please use a Command Somewhere else!**").then(msg=>{setTimeout(()=>{try{msg.delete().catch(() => {});}catch(e){ }}, 3000)})
            }
        }
        ...
        if (!channel) return message.reply("<:M_x:933261310588760115> **Please join a Voice Channel first!**").then(msg=>{setTimeout(()=>{try{msg.delete().catch(() => {});}catch(e){ }}, 5000)})
        ...
        if (player && channel.id !== player.voiceChannel) return message.reply(`<:M_x:933261310588760115> **Please join __my__ Voice Channel first! <#${player.voiceChannel}>**`).then(msg=>{setTimeout(()=>{try{msg.delete().catch(() => {});}catch(e){ }}, 3000)})
...
function generateQueueEmbed(client, guildId, leave){
    ...
    var embeds = [
      ...
      new MessageEmbed()
        ...
        .setImage(guild.banner ? guild.bannerURL({ size: 4096 }) : "https://cdn.discordapp.com/attachments/936198957921624115/936199058660409344/standard_2.gif")
        .setTitle(`Start Listening to Music, by connecting to a Voice Channel and sending either the **SONG LINK** or **SONG NAME** in this Channel!`)
        .setDescription(`> *I support <:YouTube:932982270481027082> Youtube, <:Spotify:932982195025481728> Spotify, <:soundcloud:932988353979240519> Soundcloud and direct MP3 Links!*`)
    ]
    ...
    if(!leave && player && player.queue && player.queue.current){
        embeds[1].setImage(`https://img.youtube.com/vi/${player.queue.current.identifier}/mqdefault.jpg`)
            ...
            .setAuthor(`${player.queue.current.title}`, "https://images-ext-1.discordapp.net/external/DkPCBVBHBDJC8xHHCF2G7-rJXnTwj_qs78udThL8Cy0/%3Fv%3D1/https/cdn.discordapp.com/emojis/932981891185901631.gif", player.queue.current.uri)
