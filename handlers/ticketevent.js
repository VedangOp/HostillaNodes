...
module.exports = client => {
    ...
    client.on("interactionCreate", async (interaction) => {
        ...
        if(!client.setups.has(channel.id)){
            if(interaction.customId != "ticket_verify"){
                interaction?.reply({content: "<:M_x:933261310588760115> This channel is not a Ticket", ephemeral: true})
            }
            return 
        }
        ...
        if (temptype == "close") {
            ...
            let button_ticket_verify = new MessageButton().setStyle('SUCCESS').setCustomId('ticket_verify').setLabel("Verify this Step").setEmoji("932995837003460649")
            channel.send({
                content: `<@${buttonuser.id}>`,
                embeds: [new Discord.MessageEmbed()
                ...
                collector.on('collect', async b => {
                    if (b?.user.id !== user.id)
                        return b?.reply(`<:M_x:933261310588760115> **Only the one who typed ${prefix}help is allowed to react!**`, true)
                ...
                collector.on('end', collected => {
                    if (!edited) {
                        ...
                        msg.edit({
                            content: `<@${buttonuser.id}>`,
                            embeds: [endedembed],
                            components: [new MessageActionRow().addComponents(button_ticket_verify.setDisabled(true).setLabel("FAILED TO VERIFY").setEmoji("933261310588760115").setStyle('DANGER'))]
                        }).catch((e) => {
                            console.log(String(e).grey)
                        });
                    }
            ...
            if (([...member.roles.cache.values()] && !member.roles.cache.some(r => cmdroles.includes(r.id))) && !cmdroles.includes(interaction?.user.id) && ([...member.roles.cache.values()] && !member.roles.cache.some(r => adminroles.includes(r ? r.id : r))) && !Array(guild.ownerId, config.ownerid).includes(interaction?.user.id) && !member.permissions.has("ADMINISTRATOR") && !member.roles.cache.some(r => theadminroles.includes(r ? r.id : r))&& !theadminroles.includes(member.id)) {
                return channel.send({
                    ...
                    embeds: [new MessageEmbed()
                        .setColor(es.wrongcolor)
                        .setFooter(client.getFooter(es))
                        .setTitle("<:M_x:933261310588760115> You are not allowed to delete this Ticket")
            ...
            if (([...member.roles.cache.values()] && !member.roles.cache.some(r => cmdroles.includes(r.id))) && !cmdroles.includes(interaction?.user.id) && ([...member.roles.cache.values()] && !member.roles.cache.some(r => adminroles.includes(r ? r.id : r))) && !Array(guild.ownerId, config.ownerid).includes(interaction?.user.id) && !member.permissions.has("ADMINISTRATOR") && !member.roles.cache.some(r => theadminroles.includes(r ? r.id : r))) {
                return channel.send({
                    ...
                    embeds: [new MessageEmbed()
                        .setColor(es.wrongcolor)
                        .setFooter(client.getFooter(es))
                        .setTitle("<:M_x:933261310588760115> You are not allowed to add/remove Users to/from this Ticket")
                        .setDescription(`${adminroles.length > 0 ? "You need one of those Roles: " + adminroles.map(role => `<@&${role}>`).join(" | ") + cmdrole.join(" | ") + theadminroles.join(" | ")  : `No Admin Roles Setupped yet! Do it with: \`${prefix}setup-admin\` You can also add Ticket only Roles with \`${prefix}setup-ticket\``}`)
                    ]
                });
            }
                            ...
                            if (element.id == user.id) {
                                if (!element.allow.includes("VIEW_CHANNEL")) {
                                    if(!channel.permissionsFor(channel.guild.me).has(Permissions.FLAGS.MANAGE_CHANNELS)){
                                        return channel.send(`<:M_x:933261310588760115> **I am missing the Permissions MANAGE_CHANNELS for: \`${channel.name}\`**`);
                                    }
            ...
            if (([...member.roles.cache.values()] && !member.roles.cache.some(r => cmdroles.includes(r.id))) && !cmdroles.includes(interaction?.user.id) && ([...member.roles.cache.values()] && !member.roles.cache.some(r => adminroles.includes(r ? r.id : r))) && !Array(guild.ownerId, config.ownerid).includes(interaction?.user.id) && !member.permissions.has("ADMINISTRATOR") && !member.roles.cache.some(r => theadminroles.includes(r ? r.id : r))) {
                return channel.send({
                    ...
                    embeds: [new MessageEmbed()
                        .setColor(es.wrongcolor)
                        .setFooter(client.getFooter(es))
                        .setTitle("<:M_x:933261310588760115> You are not allowed to add/remove Roles to/from this Ticket")
                        .setDescription(`${adminroles.length > 0 ? "You need one of those Roles: " + adminroles.map(role => `<@&${role}>`).join(" | ") + cmdrole.join(" | ") + theadminroles.join(" | ")  : `No Admin Roles Setupped yet! Do it with: \`${prefix}setup-admin\` You can also add Ticket only Roles with \`${prefix}setup-ticket\``}`)
                    ]
                });
            }
            ...
            if (([...member.roles.cache.values()] && !member.roles.cache.some(r => cmdroles.includes(r.id))) && !cmdroles.includes(interaction?.user.id) && ([...member.roles.cache.values()] && !member.roles.cache.some(r => adminroles.includes(r ? r.id : r))) && !Array(guild.ownerId, config.ownerid).includes(interaction?.user.id) && !member.permissions.has("ADMINISTRATOR") && !member.roles.cache.some(r => theadminroles.includes(r ? r.id : r))) {
                return channel.send({
                    ...
                    embeds: [new MessageEmbed()
                        .setColor(es.wrongcolor)
                        .setFooter(client.getFooter(es))
                        .setTitle("<:M_x:933261310588760115> You are not allowed to claim this Ticket")
                        .setDescription(`${adminroles.length > 0 ? "You need one of those Roles: " + adminroles.map(role => `<@&${role}>`).join(" | ") + cmdrole.join(" | ") + theadminroles.join(" | ")  : `No Admin Roles Setupped yet! Do it with: \`${prefix}setup-admin\` You can also add Ticket only Roles with \`${prefix}setup-ticket\``}`)
                    ]
                });
            }
            ...
            if(!channel.permissionsFor(member).has(Discord.Permissions.FLAGS.SEND_MESSAGES)){
                ...
                channel.permissionOverwrites.edit(member.user, {
                    ...
                    return interaction?.reply({ephemeral: true, content: "<:M_x:933261310588760115> **Can't change the Permissions of you!**"});
                });
            }
    ...
    client.on("interactionCreate", interaction => {
        if(interaction?.guildId && interaction?.isSelectMenu() && interaction?.message && interaction?.message.author.id == client.user.id){
            ...
            if(!DBindex) {
                ...
                if(interaction.replied) return interaction?.editReply("<:M_x:933261310588760115> Could not find the Database for your Application!");
                else return
            }
            ...
            if(message.id == settings.messageId && (channelId == settings.channelId || message.channelId == settings.channelId)){
               let index = settings.data.findIndex(v => v.value == values[0]);
                if(index < 0) {
                    return interaction?.reply({ephemeral: true, content: "<:M_x:933261310588760115> **Could not find the Ticket-Settings for this Option**"});
                }
                    ...
                    try {
                      ...
                      if(client.setups.has(ticketchannel.id) && client.setups.has(ticketchannel.id, "ticketdata"))
                      {
                        ...
                        if(data.state != "closed" && data.menutickettype == 3){
                          return interaction?.reply({content: `<:M_x:933261310588760115> **You already have an Ticket!** <#${ticketchannel.id}>`, ephemeral: true});
                        }
                      }
                    } catch {
                ...
                await interaction?.reply({content: `<a:Loading:935797567596421120> **Creating your ticket?...** (Usually takes 0-2 Seconds)`, ephemeral: true});
                try {
                ...
                if(settings.claim.enabled){
                    ...
                    .setFooter(client.getFooter(es))
                    .setAuthor(client.getAuthor(`A Staff Member will claim the Ticket soon!`, "https://cdn.discordapp.com/emojis/935797567596421120.gif?size=44", "https://discord.gg/milrato"))
                ...
                await interaction?.editReply({content: `<a:YES:932995837003460649> **Your Ticket is created!** <#${ch.id}>`, ephemeral: true});
                })
            }
        }
    })
    ...
    client.on("interactionCreate", interaction => {
        if(interaction?.guildId && interaction?.isSelectMenu() && interaction?.message && interaction?.message.author.id == client.user.id){
            ...
            if(message.id == settings.messageId && (channelId == settings.channelId || message.channelId == settings.channelId)){
                let index = settings.data.findIndex(v => v.value == values[0]);
                if(index < 0) {
                    return interaction?.reply({ephemeral: true, content: "<:M_x:933261310588760115> **Could not find the Auto-Support-Data-Settings for this Option**"});
                }
                let data = settings.data[index];
    ...
    client.on("interactionCreate", interaction => {
        if(interaction?.guildId && interaction?.isSelectMenu() && interaction?.message && interaction?.message.author.id == client.user.id){
            ...
            if(!data) return interaction?.reply({ephemeral: true, content: "<:M_x:933261310588760115> **Could not find the Data for this System**"});
