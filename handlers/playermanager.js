...
module.exports = async (client, message, args, type, slashCommand = false, extras = false) => {
  ...
  if(botchannel && channel.userLimit != 0 && channel.full){
    if(slashCommand)  return slashCommand.reply({embeds: [new MessageEmbed().setTitle("<:M_x:933261310588760115> Your Voice Channel is full!").setColor(es.wrongcolor).setFooter(client.getFooter(es))]}).catch(()=>{});
    return message.reply({embeds: [new MessageEmbed().setTitle("<:M_x:933261310588760115> Your Voice Channel is full!").setColor(es.wrongcolor).setFooter(client.getFooter(es))]}).catch(()=>{});
  }
  if (method[0] === "song")
